...
- **Double File Reads**: I read the file once in `Scanner` to calculate line numbers, throw the content away, and then read it _again_ in `ToHTML` to show snippets. That's inefficient I/O.
- **Memory Waste**: `NewLineIndex` loads the _entire file_ into memory just to find `\n` characters. On a large repo with big files, this will spike RAM unnecessarily.
